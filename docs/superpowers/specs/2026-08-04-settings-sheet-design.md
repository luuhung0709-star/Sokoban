...
| Nút `⟳ Restart` trên thanh nút | **Bỏ** (đổi ý sau khi xem thật trên trình duyệt) | Ban đầu chốt giữ cả hai. Nhìn thanh nút 5 cái trên máy thật thì thấy chật, nên bỏ. Chơi lại giờ là phím `R` hoặc hàng Restart trong sheet. Thanh còn 4 nút, vừa một hàng. Ghi lại chỗ hai đường vốn khác nhau, để ai đọc sau khỏi tưởng chúng thay thế được cho nhau: nút thanh (đã bỏ) gọi `Command.Restart` → `LevelPlayer.#restart()` (reset session tại chỗ), còn nút trong sheet gọi `flow.retryLevel()` → dựng lại cả màn qua `playLevel()`. Phím `R` đi đường thứ nhất, nên nó **chết sau overlay thắng** (`LevelPlayer.handle` trả về sớm khi session đã solved), còn nút trong sheet vẫn chạy được ở đó. |
