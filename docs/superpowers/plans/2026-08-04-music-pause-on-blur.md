...
- **Một chỗ lệch lúc viết plan, nay đã khép:** Spec mục 3 ban đầu mô tả `suspend()` là "gọi `#music.pause()`" không kèm điều kiện re-entry, nhưng bảng test đòi suspend hai lần chỉ tính một. Plan phải chốt theo bảng test — `suspend()` có `if (this.#suspended) return;`. Spec đã được sửa và bảng quyết định ghi rõ điều kiện này, nên lệch đó đã được khép.
